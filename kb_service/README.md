...
Java 后端、成员 A 的 `rag_ops.py` / `api_server.py` 保持原样，通过 HTTP 接口对接。
...
**注意**：Java 调用时必须用 `127.0.0.1`、UTF-8、HTTP/1.1（已在 `TicketController` 修复）。
...
-  `rag_ops.py`（8001）使用独立 `chroma_db/`，与 kb_service **不冲突**
...
| 问题         | 答案要点                                                          |
| ---------- | ------------------------------------------------------------- |
| 账号冻结怎么处理？  | 自助 [http://XXX.YYY.ZZZ](http://XXX.YYY.ZZZ) 或热线 400-8800-1234 |
